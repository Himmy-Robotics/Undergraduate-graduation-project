...
class HimmyVisualizer:
    ...
    def __init__(self, npy_file, output_dir="/data/zmli/Fast-Quadruped/amp_a1_jump/MetalHead/poselib/cheetah_retargetting/data/motion_vis"):
        ...
        self.fps = self.motion.fps / SLOWDOWN_FACTOR  # 应用慢放系数
        ...
        print(f"原始FPS: {self.motion.fps}, 慢放系数: {SLOWDOWN_FACTOR}x, 实际播放FPS: {self.fps}")
    ...
    def create_animation(self, save_video=True, max_frames=None):
        """创建3D动画并保存为视频（多视图：3D视图、正视图、侧视图、俯视图，显示关节角度）"""
        print("\n创建动画...")
        ...
        # 创建图形 - 使用2x3布局容纳4个视图和关节角度信息
        fig = plt.figure(figsize=(28, 14), dpi=100)
        
        # 上排：3D视图、正视图(YOZ)、侧视图(XOZ)
        ax_3d = fig.add_axes([0.02, 0.52, 0.30, 0.44], projection='3d')
        ax_front = fig.add_axes([0.34, 0.52, 0.30, 0.44], projection='3d')
        ax_side = fig.add_axes([0.66, 0.52, 0.30, 0.44], projection='3d')
        
        # 下排：俯视图(XOY)、关节角度文本区域
        ax_top = fig.add_axes([0.02, 0.04, 0.30, 0.44], projection='3d')
        ax_text = fig.add_axes([0.34, 0.04, 0.62, 0.44])
        ax_text.axis('off')  # 隐藏坐标轴
        
        # 所有3D视图的列表
        all_axes = [ax_3d, ax_front, ax_side, ax_top]
        ...
        print(f"视图: 3D视图、正视图(YOZ)、侧视图(XOZ)、俯视图(XOY)")
        ...
        for i in range(len(self.node_names)):
            ...
            if parent_idx >= 0:
                ...
                line_3d, = ax_3d.plot([], [], [], '-', color=color, linewidth=linewidth)
                line_front, = ax_front.plot([], [], [], '-', color=color, linewidth=linewidth)
                line_side, = ax_side.plot([], [], [], '-', color=color, linewidth=linewidth)
                line_top, = ax_top.plot([], [], [], '-', color=color, linewidth=linewidth)
                
                bone_lines_3d.append((i, parent_idx, line_3d))
                bone_lines_front.append((i, parent_idx, line_front))
                bone_lines_side.append((i, parent_idx, line_side))
                bone_lines_top.append((i, parent_idx, line_top))
        
        # 关节点散点（每个视图都有）
        scatter_3d = ax_3d.scatter([], [], [], c='black', s=40)
        scatter_front = ax_front.scatter([], [], [], c='black', s=40)
        scatter_side = ax_side.scatter([], [], [], c='black', s=40)
        scatter_top = ax_top.scatter([], [], [], c='black', s=40)
        
        # 轨迹线（root轨迹）- 只显示最近的轨迹（每个视图都有）
        traj_3d, = ax_3d.plot([], [], [], 'r--', alpha=0.5, linewidth=1, label='Trajectory')
        traj_front, = ax_front.plot([], [], [], 'r--', alpha=0.5, linewidth=1)
        traj_side, = ax_side.plot([], [], [], 'r--', alpha=0.5, linewidth=1)
        traj_top, = ax_top.plot([], [], [], 'r--', alpha=0.5, linewidth=1)
        trajectory_data = []
        TRAJECTORY_LENGTH = 60  # 只显示最近60帧的轨迹
        
        # 时间文本
        time_text = ax_3d.text2D(0.02, 0.98, '', transform=ax_3d.transAxes, fontsize=14, fontweight='bold')
        ...
        def init():
            ...
            ax_3d.set_title(f'3D View: {self.motion_name}', fontsize=12, fontweight='bold')
            ax_3d.legend(handles=legend_elements, loc='upper right', fontsize=8)
            
            # 设置正视图(YOZ平面)
            ax_front.set_xlabel('X (m)', fontsize=10)
            ax_front.set_ylabel('Y (m)', fontsize=10)
            ax_front.set_zlabel('Z (m)', fontsize=10)
            ax_front.set_title('Front View (YOZ)', fontsize=12, fontweight='bold')
            
            # 设置侧视图(XOZ平面)
            ax_side.set_xlabel('X (m)', fontsize=10)
            ax_side.set_ylabel('Y (m)', fontsize=10)
            ax_side.set_zlabel('Z (m)', fontsize=10)
            ax_side.set_title('Side View (XOZ)', fontsize=12, fontweight='bold')
            
            # 设置俯视图(XOY平面)
            ax_top.set_xlabel('X (m)', fontsize=10)
            ax_top.set_ylabel('Y (m)', fontsize=10)
            ax_top.set_zlabel('Z (m)', fontsize=10)
            ax_top.set_title('Top View (XOY)', fontsize=12, fontweight='bold')
            ...
            ax_front.view_init(elev=0, azim=0)      # 正视图：从X正方向看YOZ平面
            ax_side.view_init(elev=0, azim=-90)     # 侧视图：从Y正方向看XOZ平面
            ax_top.view_init(elev=90, azim=-90)     # 俯视图：从Z正方向看XOY平面
        ...
        def update(frame_idx):
            ...
            # 更新所有视图的骨骼线条
            for bone_lines in [bone_lines_3d, bone_lines_front, bone_lines_side, bone_lines_top]:
            ...
            # 更新所有视图的关节点
            for scatter in [scatter_3d, scatter_front, scatter_side, scatter_top]:
            ...
            if len(trajectory_data) > 1:
                ...
                for traj_line in [traj_3d, traj_front, traj_side, traj_top]:
            ...
            ax_3d.view_init(elev=25, azim=-60)
            ax_3d.dist = 8
            ax_front.view_init(elev=0, azim=0)      # 正视图
            ax_front.dist = 8
            ax_side.view_init(elev=0, azim=-90)     # 侧视图
            ax_side.dist = 8
            ax_top.view_init(elev=90, azim=-90)     # 俯视图
            ax_top.dist = 8
...
def main():
    ...
    parser.add_argument("--output_dir", type=str, default="/data/zmli/Fast-Quadruped/amp_a1_jump/MetalHead/poselib/cheetah_retargetting/data/motion_vis", help="输出目录")
    ...
    # 如果没有指定文件，使用retarget目录下的默认文件
    if args.npy_file is None:
        # 尝试从 load_config.json 获取文件名
        config_path = os.path.join(script_dir, "load_config.json")
        if os.path.exists(config_path):
            ...
            npy_file = os.path.join(
                script_dir, "..", "data", "cheetah_data_retarget_npy",
                f"{file_name}_amp_{clip[0]}_{clip[1]}_{remarks}.npy"
            )
        else:
            # 如果没有配置文件，尝试找到 retarget 目录下的第一个 npy 文件
            retarget_dir = os.path.join(script_dir, "..", "data", "cheetah_data_retarget_npy")
            if os.path.exists(retarget_dir):
                npy_files = [f for f in os.listdir(retarget_dir) if f.endswith('.npy')]
                if npy_files:
                    npy_file = os.path.join(retarget_dir, npy_files[0])
                    print(f"自动选择文件: {npy_files[0]}")
                else:
                    print(f"错误: retarget目录中没有npy文件: {retarget_dir}")
                    sys.exit(1)
            else:
                print(f"错误: retarget目录不存在: {retarget_dir}")
                sys.exit(1)
    else:
