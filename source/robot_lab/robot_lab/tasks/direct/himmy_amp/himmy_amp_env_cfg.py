...
MOTIONS_HIMMY_DIR = os.path.join(MOTIONS_DIR, "datasets", "mocap_motions_himmy")

# =============================================================================
# Robot Configuration Override (Himmy Mark2 for AMP)
# =============================================================================
# 复制原始配置
HIMMY_MARK2_AMP = HIMMY_MARK2_CFG.replace(prim_path="/World/envs/env_.*/Robot")
...
class HimmyAmpEnvCfg(DirectRLEnvCfg):
    ...
    Himmy Mark2 特性:
    - 15 DOF: 12 腿部关节 + 3 脊柱关节 (yaw, pitch, roll)
    - 脊柱关节提供额外的运动自由度，使运动更加自然
    - 动作数据来自 mocap_motions_himmy，包含脊柱运动
    """
    ...
    rew_termination = 0.0
    rew_lin_vel_xy = 2.0    # 线速度跟踪奖励 (原始scales值，不除以dt)
    rew_ang_vel_z = 0.8     # 角速度跟踪奖励 (原始scales值，不除以dt)
    ...
    class CommandsCfg:
        lin_vel_x_range = [0.5, 3.0]  # m/s (参考: [0.0, 5.0])
    ...
    # spaces (空间维度)
    # 新的观测空间 (包含脊柱): 
    # project_gravity(3) + commands(3) + dof_pos(15) + dof_vel(15) + actions(15) = 51
    # 比原来的 42 维增加了 9 维 (3 spine pos + 3 spine vel + 3 spine actions)
    num_observations = 51
    num_actions = 15  # 12 腿部 + 3 脊柱 (yaw, pitch, roll)
    observation_space = 51
    action_space = 15
    state_space = 0
    num_amp_observations = 1
    # AMP 观测空间 (仅使用根节点旋转和关节位置): 
    # root_rot(3) + dof_pos(15) = 18
    amp_observation_space = 18
    ...
    motion_file = None
    # 使用 Himmy Mark2 专用的动作数据集
    motions_dir = MOTIONS_HIMMY_DIR  # 包含脊柱运动的数据集
    reference_body = "base_link" # 参考身体部位 (Himmy Mark2 的基座名称)
    ...
    # robot (机器人资产配置) - 使用 Himmy Mark2
    robot: ArticulationCfg = HIMMY_MARK2_AMP
