...
from .A1_amp_env_cfg import A1AmpEnvCfg
...
class A1AmpEnv(DirectRLEnv):
    """
    A1 机器人的 AMP (Adversarial Motion Priors) 环境。
    ...
    def __init__(self, cfg: A1AmpEnvCfg, render_mode: str | None = None, **kwargs):
        """
        初始化环境。
        ...
        # 合并所有控制关节索引
        # 动作顺序: 12 个腿部关节
        self.all_joint_indexes = self.leg_joint_indexes
    ...
    def _apply_action(self):
        ...
        A1 的动作空间:
        - 12 维: 腿部关节 (FL/FR/RL/RR × hip/thigh/calf)
        """
        # self.actions 是 (N, 12) 的完整动作
        # 获取默认关节位置
        ...
        leg_actions = self.actions[:, :12]  # 12 维是腿部动作
    ...
    def _get_observations(self) -> dict:
        ...
        A1 观测空间:
        - Policy Obs (42 dim): gravity(3) + cmd(3) + dof_pos(12) + dof_vel(12) + actions(12)
        - AMP Obs (15 dim): dof_pos(12) + root_rot(3)
        """
        # 构建任务观测
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        # 5. 非脚部接触惩罚 (Undesired Contacts Penalty)
...
def compute_amp_obs(
    ...
    """
    计算 AMP 观测向量 (完整版: 包含根节点高度、位置、速度和关节信息)
    
    Args:
        dof_positions: [num_envs, 12] - 关节位置 (12 腿部)
        dof_velocities: [num_envs, 12] - 关节速度
        root_pos: [num_envs, 3] - 根节点世界位置
        root_rot: [num_envs, 4] - 根节点四元数 (xyzw)
        root_lin_vel: [num_envs, 3] - 根节点世界坐标系线速度
        root_ang_vel: [num_envs, 3] - 根节点世界坐标系角速度
        key_body_pos: [num_envs, 4, 3] - 4个脚的世界位置
    
    Returns:
        [num_envs, 46] - AMP 观测向量
    """
    root_h = root_pos[:, 2:3]
    heading_rot = calc_heading_quat_inv(root_rot)
    
    # root_rot_obs (不在local坐标系下旋转，直接提取tan_norm后3维)
    root_rot_obs = root_rot
    root_rot_obs = quat_to_tan_norm(root_rot_obs)[:, -3:]
    
    # velocity (local/root frame)
    local_root_vel = quat_rotate_inverse_amp(root_rot, root_lin_vel)
    local_root_ang_vel = quat_rotate_inverse_amp(root_rot, root_ang_vel)
    ...
    flat_end_pos = local_key_body_pos.view(-1, 3)
    flat_heading_rot = heading_rot_expand.view(-1, 4)
    
    local_end_pos = my_quat_rotate(flat_heading_rot, flat_end_pos)
    flat_local_key_pos = local_end_pos.view(local_key_body_pos.shape[0], -1)
