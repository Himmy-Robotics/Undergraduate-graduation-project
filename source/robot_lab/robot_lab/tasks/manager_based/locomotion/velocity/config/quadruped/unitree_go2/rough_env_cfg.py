...
class UnitreeGo2RoughEnvCfg(LocomotionVelocityRoughEnvCfg):
    """
    Unitree Go2 四足机器人在粗糙地形上的强化学习环境配置
    
    该配置类继承自 LocomotionVelocityRoughEnvCfg，用于定义 Go2 机器人在
    复杂地形（如起伏地面、障碍物等）上进行速度控制的运动学习任务。
    """
    # 机器人基座连杆名称
    base_link_name = "base"
    # 足部连杆名称（Go2的URDF中没有独立的foot连杆，使用calf作为足部）
    ...
    def __post_init__(self):
        ...
        self.rewards.lin_vel_z_l2.weight = -2.0           # 惩罚z方向线速度（避免跳跃）
        ...
        self.rewards.base_height_l2.params["target_height"] = 0.33  # 目标高度设置为0.33米
        ...
        self.rewards.joint_vel_l2.weight = 0              # 关节速度惩罚（已禁用）
        ...
        self.rewards.joint_power.weight = -2e-5           # 惩罚关节功率（节能）
        self.rewards.stand_still.weight = -2.0            # 惩罚静止不动
        self.rewards.joint_pos_penalty.weight = -1.0      # 关节位置惩罚
        self.rewards.joint_mirror.weight = -0.05          # 惩罚不对称步态
        # 定义对称关节对（对角腿应该镜像对称）
        ...
        self.rewards.track_lin_vel_xy_exp.weight = 3.0    # 奖励跟踪目标xy平面线速度
        self.rewards.track_ang_vel_z_exp.weight = 1.5     # 奖励跟踪目标z轴角速度

        # 其他奖励项（Others）- 主要关注足部行为
        self.rewards.feet_air_time.weight = 0.1           # 奖励足部腾空时间（鼓励动态步态）
        self.rewards.feet_air_time.params["threshold"] = 0.5  # 腾空时间阈值（秒）
        self.rewards.feet_air_time.params["sensor_cfg"].body_names = [self.foot_link_name]
        self.rewards.feet_air_time_variance.weight = -1.0  # 惩罚腾空时间方差（保持规律步态）
        ...
        self.rewards.feet_slide.weight = -0.1             # 惩罚足部滑动（避免打滑）
        ...
        self.rewards.feet_height_body.weight = -5.0       # 惩罚足部相对身体高度（避免踢到身体）
        self.rewards.feet_height_body.params["target_height"] = -0.2  # 足部相对身体目标高度（米）
        self.rewards.feet_height_body.params["asset_cfg"].body_names = [self.foot_link_name]
        self.rewards.feet_gait.weight = 0.5               # 奖励对角步态（trot gait）
        # 定义同步的足部对（对角腿同时运动）
        self.rewards.feet_gait.params["synced_feet_pair_names"] = (("FL_foot", "RR_foot"), ("FR_foot", "RL_foot"))
        self.rewards.upward.weight = 1.0                  # 奖励向上的运动方向（保持直立）
        ...
        self.curriculum.command_levels = None             # 命令级别课程（已禁用）

        # ------------------------------命令配置------------------------------
        # 可以取消注释以自定义速度命令范围
        # self.commands.base_velocity.ranges.lin_vel_x = (-1.0, 1.0)   # x方向线速度范围
        # self.commands.base_velocity.ranges.lin_vel_y = (-0.5, 0.5)   # y方向线速度范围
        # self.commands.base_velocity.ranges.ang_vel_z = (-1.0, 1.0)   # z轴角速度范围
