# Copyright (c) 2024-2025 Ziqi Fan
# SPDX-License-Identifier: Apache-2.0

"""空中翻转着陆任务配置 - Himmy Mark2 四足机器人

该任务用于训练机器人从空中落下时进行姿态调整，类似猫的翻正反射(righting reflex)。

任务特点:
- 机器人从1-3米高度自由落体
- 初始姿态完全随机(roll, pitch, yaw)
- 需要在空中利用脊柱调整身体姿态
- 目标是四脚平稳着地并保持站立

奖励设计(经过优化):
- 身体朝向奖励: 保持机器人Z轴朝上 (Core)
- 姿态平坦度: 减少roll和pitch偏离 (Core)
- 减少落地冲击力: 使用 contact_forces 惩罚
- 四脚着地: 使用 feet_contact_without_cmd 鼓励着陆
- 站立稳定: 着陆后保持静止
"""


from isaaclab.utils import configclass
from isaaclab.managers import RewardTermCfg as RewTerm
from isaaclab.managers import TerminationTermCfg as DoneTerm
from isaaclab.managers import EventTermCfg as EventTerm
from isaaclab.managers import SceneEntityCfg
from isaaclab.terrains import TerrainGeneratorCfg
from isaaclab.terrains.trimesh import MeshPlaneTerrainCfg
import math

from robot_lab.tasks.manager_based.locomotion.velocity.velocity_env_cfg import LocomotionVelocityRoughEnvCfg
import robot_lab.tasks.manager_based.locomotion.velocity.mdp as mdp

##
# Pre-defined configs
##
from robot_lab.assets._Himmy_robot import HIMMY_MARK2_CFG  # isort: skip


@configclass
class HimmyMark2AirTwistEnvCfg(LocomotionVelocityRoughEnvCfg):
    """
    Himmy Mark2 空中翻转着陆任务环境配置
    
    训练目标:
    1. 从随机高度和姿态开始自由落体
    2. 在空中利用脊柱和腿部调整姿态
    3. 四脚平稳着地，减小冲击力
    4. 着陆后保持站立不动
    """
    # 机器人基座连杆名称
    base_link_name = "base_link"
    # 足部连杆名称
    foot_link_name = ".*_foot_Link"
    
    # 关节名称列表（12个腿部关节 + 3个脊柱关节）
    joint_names = [
        "FR_hip_joint", "FR_thigh_joint", "FR_calf_joint",  # 前右腿
        "FL_hip_joint", "FL_thigh_joint", "FL_calf_joint",  # 前左腿
        "RR_hip_joint", "RR_thigh_joint", "RR_calf_joint",  # 后右腿
        "RL_hip_joint", "RL_thigh_joint", "RL_calf_joint",  # 后左腿
        "yaw_spine_joint", "pitch_spine_joint", "roll_spine_joint",  # 脊柱关节
    ]

    def __post_init__(self):
        """初始化配置参数（在类实例化后自动调用）"""
        # 调用父类的初始化方法
        super().__post_init__()

        # ------------------------------场景配置------------------------------
        # 设置机器人模型配置和在场景中的路径
        self.scene.robot = HIMMY_MARK2_CFG.replace(prim_path="{ENV_REGEX_NS}/Robot")
        
        # --- Flat Environment Settings ---
        # 将地形改为平面
        self.scene.terrain.terrain_type = "plane"          # 使用简单平面地形
        self.scene.terrain.terrain_generator = None        # 不使用地形生成器
        
        # 移除高度扫描（平坦地形不需要感知地形起伏）
        self.scene.height_scanner = None                   # 禁用场景高度扫描器
        self.scene.height_scanner_base = None              # 禁用基座高度扫描器
        
        # Disable remote assets to prevent network timeouts
        self.scene.sky_light.spawn.texture_file = None
        self.scene.terrain.visual_material = None

        # ------------------------------Episode配置------------------------------
        # 空中翻转任务的episode长度：设置为2.5秒，加快重置频率以便于观察多次尝试
        self.episode_length_s = 3
        # 设置观察值的缩放因子（用于归一化输入到神经网络的数据）
        self.observations.policy.base_lin_vel.scale = 2.0       # 基座线速度缩放
        self.observations.policy.base_ang_vel.scale = 0.25      # 基座角速度缩放
        self.observations.policy.joint_pos.scale = 1.0          # 关节位置缩放
        self.observations.policy.joint_vel.scale = 0.05         # 关节速度缩放
        
        # 禁用某些观察值（设为 None）
        self.observations.policy.base_lin_vel = None            # 不使用基座线速度观察
        self.observations.policy.height_scan = None             # 不使用高度扫描观察
        self.observations.critic.height_scan = None             # 禁用价值网络的高度扫描观察

        # 指定需要观察的关节
        self.observations.policy.joint_pos.params["asset_cfg"].joint_names = self.joint_names
        self.observations.policy.joint_vel.params["asset_cfg"].joint_names = self.joint_names
        self.observations.critic.joint_pos.params["asset_cfg"].joint_names = self.joint_names   
        self.observations.critic.joint_vel.params["asset_cfg"].joint_names = self.joint_names   
        # ------------------------------动作配置------------------------------
        # 设置动作缩放（减小动作幅度以提高稳定性）
        self.actions.joint_pos.scale = {
            ".*_hip_joint": 0.125, 
            ".*_spine_joint": 0.25, 
            "^(?!.*(_hip_joint|_spine_joint)).*": 0.25
        }
        # 设置动作裁剪范围
        self.actions.joint_pos.clip = {".*": (-100.0, 100.0)}
        # 指定动作控制的关节
        self.actions.joint_pos.joint_names = self.joint_names

        # ------------------------------事件配置------------------------------
        # 配置环境重置时的随机化参数（用于域随机化，增强策略鲁棒性）
        self.events.randomize_reset_base.params = {
            "pose_range": {  # 位姿随机化范围
                "x": (-0.0, 0.0),          # x方向位置（米）
                "y": (-0.0, 0.0),          # y方向位置（米）
                "z": (2.0, 2.0),           # z方向位置（米）
                "roll": (3.14, 3.14),     # 滚转角（弧度）
                "pitch": (0.0, 0.0),    # 俯仰角（弧度）
                "yaw": (0, 0),      # 偏航角（弧度）
            },
            "velocity_range": {  # 速度随机化范围
                "x": (-0.0, 0.0),          # x方向线速度（米/秒）
                "y": (-0.0, 0.0),          # y方向线速度（米/秒）
                "z": (-0.0, 0.0),          # z方向线速度（米/秒）
                "roll": (-0.0, 0.0),       # 滚转角速度（弧度/秒）
                "pitch": (-0.0, 0.0),      # 俯仰角速度（弧度/秒）
                "yaw": (-0.0, 0.0),        # 偏航角速度（弧度/秒）
            },
        }
        # 随机化基座质量
        self.events.randomize_rigid_body_mass_base.params["asset_cfg"].body_names = [self.base_link_name]
        # 随机化其他刚体质量（除基座外）
        self.events.randomize_rigid_body_mass_others.params["asset_cfg"].body_names = [
            f"^(?!.*{self.base_link_name}).*"
        ]
        # 随机化质心位置
        self.events.randomize_com_positions.params["asset_cfg"].body_names = [self.base_link_name]
        # 随机施加外力和力矩（模拟外部扰动）
        self.events.randomize_apply_external_force_torque.params["asset_cfg"].body_names = [self.base_link_name]


        # ------------------------------奖励配置------------------------------

        # 通用奖励
        self.rewards.is_terminated.weight = 0  # 终止惩罚权重（已禁用）



        self.rewards.joint_pos_limits.weight = -5.0       # 惩罚接近关节位置极限

        self.rewards.joint_pos_penalty.weight = -2.0      # 关节位置惩罚

        self.rewards.action_rate_l2.weight = -0.001        # 惩罚动作变化率（平滑控制）


        self.rewards.undesired_contacts.weight = -4.0     # 惩罚非足部接触（如膝盖、身体触地）
        self.rewards.undesired_contacts.params["sensor_cfg"].body_names = [f"^(?!.*{self.foot_link_name}).*"]
        self.rewards.contact_forces.weight = -1.5e-4      # 惩罚过大的接触力
        self.rewards.contact_forces.params["sensor_cfg"].body_names = [self.foot_link_name]

        self.rewards.upward.weight = 4.0                  # 奖励向上的运动方向（保持直立）

        self.rewards.flat_orientation_l2.weight = -4       # 姿态平坦度惩罚（已禁用）

        self.rewards.feet_contact_without_cmd.weight = 2  # 奖励无运动命令时的足部接触（稳定站立）
        self.rewards.feet_contact_without_cmd.params["sensor_cfg"].body_names = [self.foot_link_name]




        self.rewards.feet_contact_without_cmd.weight = 5  # 奖励无运动命令时的足部接触（稳定站立）
        self.rewards.track_lin_vel_xy_exp.weight = 3.0    # 奖励跟踪目标xy平面线速度
        self.rewards.track_ang_vel_z_exp.weight = 1.5     # 奖励跟踪目标z轴角速度

        self.rewards.joint_acc_l2.weight = -1.5e-7        # 惩罚关节加速度（平滑运动）
        self.rewards.joint_acc_l2.params["asset_cfg"].joint_names = ["^(?!.*_spine_joint).*"]



        # 如果奖励权重为0，则禁用该奖励项
        if self.__class__.__name__ == "HimmyMark2AirTwistEnvCfg":
            self.disable_zero_weight_rewards()

        # ------------------------------终止条件配置------------------------------
        # 禁用非法接触终止（因为我们期望着陆，虽然undesired_contacts会罚分，但可以不终止以允许恢复尝试，或者直接终止）
        # 这里选择不立即终止，让它尝试站起来，除非身体长时间触地？
        # 一般Isaac Lab的illegal_contact是指身体触地。
        # 为了加快训练，如果身体触地可以直接重置。
        self.terminations.illegal_contact = DoneTerm(
             func=mdp.illegal_contact,
             params={
                 "threshold": 1.0,
                 "sensor_cfg": SceneEntityCfg("contact_forces", body_names=[self.base_link_name]),
             },
        )
        
        # ------------------------------课程学习配置------------------------------
        # 空中翻转任务不需要课程学习
        self.curriculum.command_levels = None
        self.curriculum.terrain_levels = None

        # ------------------------------命令配置------------------------------
        # 目标命令始终为0 (原地站立)
        self.commands.base_velocity.ranges.lin_vel_x = (2.0, 2.0)
        self.commands.base_velocity.ranges.lin_vel_y = (-0.0, 0.0)
        self.commands.base_velocity.ranges.ang_vel_z = (-0.0, 0.0)




