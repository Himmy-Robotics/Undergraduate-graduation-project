...
class AMPLoader:
    ...
    def __init__(
        ...
        self.trajectories_amp = []  # AMP 观测轨迹（不含脚趾位置）
    ...
    def get_frame_at_time(self, traj_idx, time):
        """Returns AMP observation frame (without toe positions and root height) for the given trajectory at the specified time.
        
        Returns 30 dim: Joint Pos (12) + Lin Vel (3) + Ang Vel (3) + Joint Vel (12)
        
        注意：移除了 root height，原因：
        - 数据集来自 A1 机器人（root height ~0.30m）
        - Himmy Mark2 正常站高约 0.40m
        - 如果判别器要求匹配 A1 的 0.30m 高度，Himmy 只能"跪下"才能达到
        - 移除 height 后，判别器只关注动作风格（关节角度+速度），不约束绝对高度
        """
        ...
        blend = p * n - idx_low
        # Interpolate (不再添加 root height)
        frame_blend = self.slerp(frame_start, frame_end, blend)
        return frame_blend
    ...
    def feed_forward_generator(self, num_mini_batch, mini_batch_size):
        ...
        AMP 观测格式 (30 dim，不含脚趾位置和root height):
        - Joint Pos (12): index 7-19 in raw data
        - Linear Vel (3): index 31-34 in raw data  
        - Angular Vel (3): index 34-37 in raw data
        - Joint Vel (12): index 37-49 in raw data
        
        注意：移除了：
        - Toe Pos Local (index 19-31)：脚位置依赖机器人腿长
        - Root Height (index 2)：数据集来自A1，Himmy站高不同
        """
